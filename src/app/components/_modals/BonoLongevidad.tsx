...
function BonoLongevidad({ isOpen, onClose }: ModalProps) {
  return (
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-[9999] flex justify-center items-center p-4">
          ...
          <motion.div initial={{ opacity: 0, y: 30, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 20 }} transition={{ duration: 0.3, ease: 'easeOut' }} className="relative z-10 bg-white rounded-3xl shadow-elevated max-w-md w-full p-7" onClick={(e) => e.stopPropagation()}>
            ...
            <h3 className="text-xl font-bold text-slate-900 mb-3">Seguro de longevidad</h3>
            <p className="text-slate-600 leading-relaxed">Es una propuesta para que parte de las cotizaciones se destinen exclusivamente para cuando se pasa la expectativa de vida esperada.</p>
